...
| 3 | release 编译 + `SelfTest` 自检 | 任一 FAIL 不出包；通用包还要求两个切片各自过一遍 |
...
| 价格 | 「价格与销售范围」→ 价格时间表，**默认是空的** | 空着就是没设价，提交会卡在「未选择价格」；这个 App 选 Free。这一格不在版本页上，逐格复查版本页永远查不出来 |
| 供应情况 + 欧盟 DSA | 同一页的「App 供应情况」，以及「商务」页的合规问答 | 两处是一对：供应情况默认勾全部地区（含欧盟商店），勾了就必须答 DSA 交易者问卷，而那份问卷的答案会公开显示在产品页上。反过来只答不解绑也没用，两步配套才行 |
| 皮肤可用性 | 六套皮肤全部随包可用，界面上没有解锁入口 | 挂着「解锁」按钮却直接放行是审核指南 2.1 的明确拒点，所以那一层压根不渲染（`Channel.showsPricing`，见 `docs/DESIGN.md`） |

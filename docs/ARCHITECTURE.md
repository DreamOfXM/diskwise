...
│   │   │   ├── Skins.swift        #   6 套皮肤数据（免费 3 + 付费 3），加皮肤只加这里
│   │   │   ├── ThemeManager.swift #   单例：持久化 + 试穿 + canUse/unlock（StoreKit 唯一接缝）
...
- `Theme` 是纯数据，但**不只颜色**：`face`（圆体/衬线/几何）、`elevation`（浮起/描边/玻璃）、`backdrop`（纯色/渐变/极光/宣纸纹）、`metric`（圆角/行高/间距）、`motion`（动效签名）、`tileShape` + `tileStrategy`（图标块形状与取色策略）。付费皮肤靠这些**结构差异**卖，靠颜色卖不掉。
...
- 现有 6 套：免费 `dawn` 晨雾（默认）· `graphite` 石墨 · `mint` 薄荷；付费 `midnight` 极夜黑金 · `aurora` 极光玻璃 · `inkwash` 水墨宣纸。
- 试穿 `startTrying/stopTrying` 只改 `effective`，不写偏好，切走即还原。
- 付费桩：`ThemeManager.canUse/unlock` + `PaywallSheet`。**接 StoreKit 时只换这两处**，视图不动。
...
| 外观皮肤 | `ThemeManager` | 皮肤商店：每张卡带实时缩略微组件；付费靠结构差异；试穿先上身；明暗三选；付费走 `PaywallSheet` 桩 |

---

## 6. 已知缺口（按建议顺序修）

1. **StoreKit 未接入**：`canUse/unlock` + `PaywallSheet` 是桩，点「解锁」直接放行（方便看效果，也意味着付费皮肤白送）。上真正的商店前必须补：接 StoreKit 2 非消耗型商品，只换这两处。
...
- `docs/DESIGN.md`：皮肤商业化规则（阵容 / 定价、「付费靠骨架差异不靠颜色」、图标块 `tileStrategy`、`chart[]` 位语义）+ 两条跨皮肤铁律：**正文禁染色**（只用 `palette.ink` / `inkSecondary`，彩色只给图标块、环形图、按钮、徽章）；**页头 / banner 禁整块高饱和底色**。token 的权威定义始终在代码：`Sources/DiskCleaner/Theme/Skins.swift`。
